<x-app-layout>
    ...
    <div class="print-header" style="text-align:center;margin-bottom:20px;padding-bottom:12px;border-bottom:2px solid #111">
        ...
        <div style="font-size:9pt;color:#555;margin-top:2px">Dicetak pada: {{ now()->translatedFormat('d F Y, H:i') }} WIB</div>
    </div>
    ...
    <div class="no-print" style="background:linear-gradient(135deg,#0f4c2a 0%,#15803d 60%,#16a34a 100%);border-radius:16px;overflow:hidden;margin-bottom:24px;position:relative;padding:28px 32px;">
        ...
        <div style="position:relative;display:flex;align-items:center;justify-content:space-between;flex-wrap:wrap;gap:16px">
            <div>
                ...
                <h1 style="color:#fff;font-size:1.5rem;font-weight:800;letter-spacing:-.02em;margin-bottom:6px">Laporan Pengajuan Surat</h1>
                <p style="color:rgba(255,255,255,.75);font-size:.85rem">Rekap jumlah pengajuan surat beserta nama pemohon per periode.</p>
            </div>
            {{-- Tombol Cetak --}}
            <a id="btn-cetak"
               href="{{ route('admin.laporan.cetak', array_filter(['bulan'=>$bulan,'tahun'=>$tahun,'nama'=>$nama,'status'=>$status,'jenis_surat_id'=>$jenisSuratId])) }}"
               target="_blank"
    ...
    <div class="g-card no-print" style="border:none;box-shadow:0 1px 4px rgba(0,0,0,.07);margin-bottom:20px;padding:20px 24px">
        <form method="GET" action="{{ route('admin.laporan.index') }}" style="display:flex;align-items:flex-end;gap:12px;flex-wrap:wrap">
            {{-- Bulan --}}
            ...
            <div style="display:flex;flex-direction:column;gap:4px;flex:1;min-width:180px">
                ...
                <div style="position:relative">
                    ...
                    <input type="text" name="nama" id="filter-nama" value="{{ $nama }}"
            ...
            <div style="display:flex;flex-direction:column;gap:4px;min-width:160px">
                ...
                <select name="status" id="filter-status"
                    ...
                    <option value="Menunggu Kades" {{ $status == 'Menunggu Kades'  ? 'selected' : '' }}>Menunggu Kades</option>
            ...
            <div style="display:flex;gap:8px;align-items:flex-end">
                <button type="submit" id="btn-filter-laporan"
    ...
    <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:14px;margin-bottom:20px">
        @php
        $cards = [
            ['label'=>'Total Pengajuan',  'value'=>$summary->total     ?? 0, 'color'=>'#7c3aed','bg'=>'#f5f3ff'],
            ['label'=>'Disetujui',         'value'=>$summary->disetujui ?? 0, 'color'=>'#16a34a','bg'=>'#f0fdf4'],
            ['label'=>'Menunggu',          'value'=>$summary->menunggu  ?? 0, 'color'=>'#d97706','bg'=>'#fffbeb'],
    ...
    <div class="g-card" style="border:none;box-shadow:0 1px 4px rgba(0,0,0,.07)">
        <div style="padding:16px 20px;border-bottom:1px solid #f3f4f6">
            ...
            <p style="font-size:.75rem;color:#6b7280;margin-top:2px" class="print-only" style="display:none">Periode: {{ $periode->translatedFormat('F Y') }}</p>
        </div>
            ...
            <table style="width:100%;border-collapse:collapse;font-size:.83rem">
                <thead>
                    <tr style="background:#f9fafb;border-bottom:2px solid #e5e7eb">
                        ...
                        <th style="padding:10px 18px;text-align:left;font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#6b7280" class="no-print">Nama Pemohon</th>
                        ...
                        <th style="padding:10px 18px;text-align:center;font-size:.68rem;font-weight:700;text-transform:uppercase;letter-spacing:.06em;color:#6b7280">Menunggu</th>
                ...
                <tbody>
                    ...
                    @forelse($dataAktif as $j)
                        ...
                        <tr onclick="togglePemohon('{{ $rowId }}')"
                            ...
                            {{-- Kolom Pemohon --}}
                            <td style="padding:12px 18px;vertical-align:top" class="no-print">
                                {{-- Preview singkat --}}
                                    ...
                                    <div style="display:flex;flex-direction:column;gap:3px">
                                        @foreach($j->pengajuanSurats as $idx => $p)
                                            @php
                                                ...
                                                $nikPemohon  = $p->user->penduduk->nik  ?? null;
                                                $sc = match($p->status) {
                                            ...
                                            @endphp
                                            <div style="display:flex;align-items:center;gap:7px;padding:5px 8px;background:#f9fafb;border-radius:7px;border:1px solid #f3f4f6">
                                                ...
                                                <div style="flex:1;min-width:0">
                                                    <div style="font-size:.79rem;font-weight:600;color:#111827">{{ $namaPemohon }}</div>
                                                    @if($nikPemohon)
                                                    <div style="font-size:.67rem;color:#9ca3af;font-family:monospace">{{ $nikPemohon }}</div>
                                                    @endif
                                                </div>
                                                <span style="font-size:.65rem;color:#9ca3af;white-space:nowrap">{{ \Carbon\Carbon::parse($p->tanggal_pengajuan)->format('d M Y') }}</span>
                            ...
                            <td style="padding:12px 18px;text-align:center;vertical-align:top">
                                <span style="display:inline-block;padding:2px 10px;border-radius:99px;font-size:.78rem;font-weight:700;background:#fef3c7;color:#b45309">{{ $j->menunggu }}</span>
                            </td>
                            <td style="padding:12px 18px;text-align:center;vertical-align:top">
                    ...
                    @empty
                        <tr>
                            <td colspan="7" style="text-align:center;padding:56px 0;color:#9ca3af">
                                ...
                                @else
                                    <p style="font-weight:600;font-size:.9rem">Tidak ada pengajuan surat pada periode ini.</p>
        ...
        <div style="padding:12px 20px;border-top:1px solid #f3f4f6;font-size:.75rem;color:#9ca3af" class="no-print">
            ...
            @if($nama || $status) yang cocok dengan filter pencarian
            @else pada periode <strong style="color:#374151">{{ $periode->translatedFormat('F Y') }}</strong>
            @endif.
            Klik baris untuk melihat nama pemohon.
        </div>
        {{-- Footer cetak --}}
        <div style="display:none;padding:12px 20px;border-top:1px solid #ccc;font-size:9pt;color:#555;margin-top:20px" class="print-footer">
