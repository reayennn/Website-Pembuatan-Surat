<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center justify-between">
            <nav class="flex items-center gap-2 text-sm">
                ...
                <span class="text-gray-300">/</span>
                <span class="font-semibold text-gray-700">Tambah Baru</span>
            </nav>
    ...
    <style>
        ...
        .a4-kop-text { flex: 1; text-align: center; line-height: 1.3; }
    ...
    <div class="py-8">
        <div class="mx-auto px-4 sm:px-6 lg:px-8" style="max-width: 1360px;">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
                ...
                <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
                    ...
                    <div class="flex items-center gap-3 px-6 py-4 bg-green-700">
                        ...
                        <div>
                            <h2 class="text-white font-bold text-sm">Tambah Template Surat</h2>
                            <p class="text-green-200 text-xs">Isi data dan konten surat</p>
                        </div>
                    </div>

                    {{-- Form --}}
                    <form id="templateForm" action="{{ route('admin.template_surat.store') }}" method="POST" class="p-6 space-y-4">
                        @csrf
                        ...
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                <label class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1.5">Jenis Surat <span class="text-red-500">*</span></label>
                                <select name="jenis_surat_id" class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-green-500 bg-white" required>
                                    <option value="">— Pilih —</option>
                                    @foreach($jenisSurats as $j)
                                        <option value="{{ $j->id }}" {{ old('jenis_surat_id') == $j->id ? 'selected' : '' }}>{{ $j->nama_surat }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div>
                        ...
                        <div>
                            ...
                            <textarea name="keterangan" rows="2" placeholder="Deskripsi singkat surat ini..."
                                      class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-green-500 resize-none">{{ old('keterangan') }}</textarea>
                        </div>

                        {{-- Info kotak --}}
                        <div class="p-3 bg-blue-50 border border-blue-200 rounded-xl">
                            <p class="text-xs font-bold text-blue-700 mb-1">Info: Cara Buat Template</p>
                            <ul class="text-xs text-blue-600 space-y-0.5 list-disc list-inside">
                                <li><span class="font-semibold text-green-700">Paragraf Pembuka & Penutup</span> — diketik manual</li>
                                <li><span class="font-semibold text-amber-700">Tabel Data Masyarakat</span> — otomatis dari sistem</li>
                            </ul>
                        </div>
                        ...
                        <div class="p-3 bg-amber-50 border border-amber-200 rounded-xl">
                            ...
                            <p class="text-xs text-amber-500 italic mt-1.5">Tabel ini tidak perlu diketik — diisi otomatis saat surat dicetak.</p>
                        </div>
                        ...
                        <div>
                            ...
                            <textarea name="persyaratan" rows="2" placeholder="Contoh: Fotokopi KTP, Surat pengantar RT/RW..."
                                      class="w-full px-3.5 py-2.5 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-green-500 resize-none">{{ old('persyaratan') }}</textarea>
                        </div>
                        ...
                        <div class="a4-paper mx-auto" style="max-width: 560px;">
                            ...
                            <div class="a4-kop">
                                ...
                                <div class="a4-kop-text">
                                    <p class="k1">{{ $kopSurat->baris_1 ?: 'PEMERINTAH KABUPATEN' }}</p>
                                    <p class="k2">{{ $kopSurat->baris_2 ?: 'KECAMATAN' }}</p>
                                    <p class="k3">{{ $kopSurat->baris_3 ?: 'DESA KAROMBO' }}</p>
                                    <p class="k4">{{ $kopSurat->baris_4 ?: 'Alamat Desa' }}</p>
                                </div>
                            </div>

                            {{-- Judul --}}
                            <div class="a4-judul">
                                <p class="title" id="p_judul">JUDUL SURAT</p>
                                <p class="nomor">Nomor : _____ / _____ / _____ / _____</p>
                            </div>
    ...
    (function () {
        quillPembuka = new Quill('#quill-pembuka', { theme: 'snow', modules: { toolbar: toolbarOptions }, placeholder: 'Ketik paragraf pembuka di sini...' });
        quillPenutup = new Quill('#quill-penutup', { theme: 'snow', modules: { toolbar: toolbarOptions }, placeholder: 'Ketik paragraf penutup di sini...' });
