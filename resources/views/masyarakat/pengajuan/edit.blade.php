<x-app-layout>
    <x-slot name="header">Perbaikan Pengajuan Surat</x-slot>

    {{-- Page header --}}
    <div style="background:#d97706;border-radius:16px;padding:22px 28px;margin-bottom:20px;position:relative;overflow:hidden">
        ...
        <div style="position:relative;display:flex;align-items:center;justify-content:space-between">
            <div>
                ...
                <p style="color:rgba(255,255,255,.9);font-size:.78rem;margin-top:2px">Perbaiki form sesuai dengan catatan dari admin desa.</p>
            </div>
            <div style="font-size:.72rem;color:rgba(255,255,255,.5)">
                ...
                <span style="color:#fff">Perbaikan</span>
            </div>
        </div>
    </div>

    {{-- Keterangan Perbaikan dari Admin --}}
    @if($pengajuan->keterangan)
        <div style="background:#fffbeb;border:1.5px solid #fde68a;border-radius:10px;padding:14px 18px;margin-bottom:20px;display:flex;align-items:flex-start;gap:12px">
            <svg width="20" height="20" fill="none" stroke="#d97706" viewBox="0 0 24 24" style="flex-shrink:0;margin-top:2px"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>
            <div>
                <p style="font-weight:700;color:#92400e;font-size:.85rem;margin-bottom:2px">Catatan Perbaikan dari Admin:</p>
                <p style="color:#b45309;font-size:.8rem;line-height:1.5">{{ $pengajuan->keterangan }}</p>
            </div>
        </div>
    @endif
    ...
    @if(session('error'))
        <div style="background:#fef2f2;border:1.5px solid #fca5a5;border-radius:10px;padding:14px 18px;margin-bottom:16px;display:flex;align-items:flex-start;gap:12px">
            ...
            <div>
                <p style="font-weight:700;color:#b91c1c;font-size:.82rem">Terjadi Kesalahan</p>
    ...
    <div style="display:grid;grid-template-columns:280px 1fr 1fr;gap:16px;align-items:start">
        ...
        <div style="display:flex;flex-direction:column;gap:16px">
            ...
            {{-- Pilih Jenis Surat (DISABLED saat perbaikan) --}}
            <div class="g-card">
                <div style="padding:12px 16px;background:#6b7280;border-radius:14px 14px 0 0;display:flex;align-items:center;justify-content:space-between">
                    <p style="color:#fff;font-size:.78rem;font-weight:700">(1) Jenis Surat</p>
                    <span style="font-size:.68rem;background:rgba(255,255,255,.2);color:#fff;padding:2px 8px;border-radius:99px;font-weight:600">Tidak dapat diubah</span>
                </div>
                <div style="padding:14px">
                    <select id="jenis-surat-select" class="f-input" style="font-size:.82rem;background:#f3f4f6;color:#6b7280;cursor:not-allowed;opacity:.75" disabled>
                        @foreach($jenisSurats as $j)
                            <option value="{{ $j->id }}" {{ $pengajuan->jenis_surat_id == $j->id ? 'selected' : '' }}>
                                {{ $j->nama_surat }}
                            </option>
                        @endforeach
                    </select>
            ...
            {{-- Edit Data Surat --}}
            <div class="g-card" id="panel-data" style="opacity:1;pointer-events:all;transition:all .3s">
                <div style="padding:12px 16px;background:#b45309;border-radius:14px 14px 0 0">
                    <p id="label-data-utama" style="color:#fff;font-size:.78rem;font-weight:700">(2) Edit Data (Opsional)</p>
                </div>
                <div style="padding:14px;display:flex;flex-direction:column;gap:10px">
                    <p style="font-size:.68rem;color:#9ca3af;margin-bottom:2px;line-height:1.5">Perbaiki data yang diperlukan (Kosongkan bila tidak ada perubahan).</p>

                    @php 
                        $p = Auth::user()->penduduk; 
                        $dp = $pengajuan->data_pemohon ?? [];
                    @endphp

                    <div>
                        <label class="f-label" style="font-size:.65rem">Nama</label>
                        <input id="f-nama" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('nama', $dp['nama'] ?? $p->nama) }}" placeholder="Nama lengkap">
                    </div>
                    <div>
                        <label class="f-label" style="font-size:.65rem">NIK</label>
                        <input id="f-nik" class="f-input" style="font-size:.8rem;padding:7px 10px;background:#f9fafb;color:#6b7280" type="text" value="{{ old('nik', $dp['nik'] ?? $p->nik) }}" readonly>
                    </div>
                    <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
                        <div>
                            <label class="f-label" style="font-size:.65rem">Tempat Lahir</label>
                            <input id="f-tempat" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('tempat_lahir', $dp['tempat_lahir'] ?? $p->tempat_lahir) }}" placeholder="Kota">
                        </div>
                        <div>
                            <label class="f-label" style="font-size:.65rem">Tgl. Lahir</label>
                            <input id="f-tgl" class="f-input" style="font-size:.8rem;padding:7px 10px" type="date" value="{{ old('tanggal_lahir', $dp['tanggal_lahir'] ?? $p->tanggal_lahir) }}">
                        </div>
                    </div>
                    <div>
                        ...
                        <select id="f-gender" class="f-input" style="font-size:.8rem;padding:7px 10px">
                            @php $gender = old('jenis_kelamin', $dp['jenis_kelamin'] ?? $p->jenis_kelamin); @endphp
                    ...
                    <div>
                        ...
                        <div id="f-status-kawin-display" style="width:100%;padding:7px 10px;background:#f9fafb;border:1.5px solid #e5e7eb;border-radius:8px;font-size:.8rem;color:#374151;cursor:not-allowed">
                            {{ old('status_perkawinan', $dp['status_perkawinan'] ?? $p->status_perkawinan ?? 'Belum Kawin') }}
                        </div>
                    </div>
                    <div>
                        <label class="f-label" style="font-size:.65rem">Agama</label>
                        <input id="f-agama" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('agama', $dp['agama'] ?? $p->agama) }}" placeholder="Agama">
                    </div>
                    <div>
                        <label class="f-label" style="font-size:.65rem">Pekerjaan</label>
                        <input id="f-pekerjaan" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('pekerjaan', $dp['pekerjaan'] ?? $p->pekerjaan) }}" placeholder="Pekerjaan">
                    </div>
                    <div>
                        <label class="f-label" style="font-size:.65rem">Alamat</label>
                        <textarea id="f-alamat" class="f-input" style="font-size:.8rem;padding:7px 10px;resize:vertical" rows="2" placeholder="Alamat lengkap">{{ old('alamat', $dp['alamat'] ?? $p->alamat) }}</textarea>
                    </div>
                    <div>
                        <label class="f-label" style="font-size:.65rem">Keperluan / Tujuan <span style="color:#dc2626">*</span></label>
                        <textarea id="f-keperluan" class="f-input" style="font-size:.8rem;padding:7px 10px;resize:vertical" rows="3" placeholder="Untuk keperluan..." minlength="5">{{ old('keperluan', $pengajuan->keperluan) }}</textarea>
                    ...
                    <div id="panel-kematian" style="display:none;margin-top:20px;border-top:1px dashed #d1d5db;padding-top:14px">
                        ...
                        <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px">
                            <div>
                                ...
                                <input id="f-kem-hari" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('kematian_hari', $dp['kematian_hari'] ?? '') }}" placeholder="Contoh: Senin">
                            </div>
                            <div>
                                <label class="f-label" style="font-size:.65rem">Tanggal</label>
                                <input id="f-kem-tgl" class="f-input" style="font-size:.8rem;padding:7px 10px" type="date" value="{{ old('kematian_tanggal', $dp['kematian_tanggal'] ?? '') }}">
                            </div>
                        </div>
                        <div style="margin-top:8px">
                            <label class="f-label" style="font-size:.65rem">Tempat Meninggal</label>
                            <input id="f-kem-tempat" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('kematian_tempat', $dp['kematian_tempat'] ?? '') }}" placeholder="Contoh: RSUD Dompu">
                        </div>
                        <div style="margin-top:8px">
                            <label class="f-label" style="font-size:.65rem">Penyebab Kematian</label>
                            <input id="f-kem-sebab" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('kematian_penyebab', $dp['kematian_penyebab'] ?? '') }}" placeholder="Contoh: Sakit">
                        </div>
                        ...
                        <div style="margin-top:8px">
                            ...
                            <input id="f-pel-nama" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('pelapor_nama', $dp['pelapor_nama'] ?? '') }}" placeholder="Nama pelapor">
                        </div>
                        <div style="margin-top:8px">
                            <label class="f-label" style="font-size:.65rem">Nomor KTP</label>
                            <input id="f-pel-ktp" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('pelapor_nik', $dp['pelapor_nik'] ?? '') }}" placeholder="NIK pelapor">
                        </div>
                        <div style="margin-top:8px">
                            <label class="f-label" style="font-size:.65rem">Tempat Tgl Lahir / Umur</label>
                            <input id="f-pel-ttl" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('pelapor_ttl', $dp['pelapor_ttl'] ?? '') }}" placeholder="Contoh: Dompu, 1 Januari 1990">
                        </div>
                        <div style="margin-top:8px">
                            <label class="f-label" style="font-size:.65rem">Pekerjaan</label>
                            <input id="f-pel-kerja" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('pelapor_pekerjaan', $dp['pelapor_pekerjaan'] ?? '') }}" placeholder="Pekerjaan pelapor">
                        </div>
                        <div style="margin-top:8px">
                            <label class="f-label" style="font-size:.65rem">Alamat</label>
                            <textarea id="f-pel-alamat" class="f-input" style="font-size:.8rem;padding:7px 10px;resize:vertical" rows="2" placeholder="Alamat pelapor">{{ old('pelapor_alamat', $dp['pelapor_alamat'] ?? '') }}</textarea>
                        </div>
                        <div style="margin-top:8px">
                            <label class="f-label" style="font-size:.65rem">Hubungan Pelapor dengan Yang Meninggal</label>
                            <input id="f-pel-hub" class="f-input" style="font-size:.8rem;padding:7px 10px" type="text" value="{{ old('pelapor_hubungan', $dp['pelapor_hubungan'] ?? '') }}" placeholder="Contoh: Anak Kandung, Suami/Istri">
                        </div>
                    </div>
                </div>
            </div>
            ...
            {{-- Submit --}}
            <div id="panel-submit" style="opacity:1;pointer-events:all;transition:all .3s">
                ...
                <form action="{{ route('pengajuan.update', $pengajuan->id) }}" method="POST" id="form-pengajuan" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="g-card" style="margin-bottom:16px;padding:14px;background:#fffbeb;border:1.5px solid #fde68a">
                        <p style="font-size:.78rem;font-weight:700;color:#92400e;margin-bottom:6px">Persyaratan Surat</p>
                        <div id="syarat-container" style="font-size:.75rem;color:#b45309;margin-bottom:12px;line-height:1.5">
                            Pilih jenis surat untuk melihat persyaratan.
                        </div>

                        {{-- Area Upload Multi-File --}}
                        <div style="border:2px dashed #fbbf24;border-radius:10px;padding:14px;background:#fff;transition:border-color .2s" id="upload-area">
                            <p style="font-size:.72rem;font-weight:700;color:#92400e;margin-bottom:4px">Upload Berkas Persyaratan Baru (Opsional)</p>
                            <p style="font-size:.67rem;color:#b45309;margin-bottom:10px;line-height:1.5">
                                Jika berkas lebih dari 1, pilih satu per satu.<br>
                                Format: PDF, JPG, PNG &bull; Maks. 5MB per berkas &bull; Maks. 5 berkas.<br>
                                Biarkan kosong jika tidak ingin mengganti berkas lama.
                            </p>

                            {{-- Tampilkan berkas lama jika ada --}}
                            @if($pengajuan->file_persyaratan)
                            <div style="display:flex;align-items:center;gap:8px;background:#fef3c7;border:1.5px solid #fde68a;border-radius:8px;padding:6px 10px;font-size:.72rem;margin-bottom:10px">
                                <span style="background:#d97706;color:#fff;border-radius:6px;padding:2px 7px;font-size:.65rem;font-weight:700">LAMA</span>
                                <span style="flex:1;color:#92400e;overflow:hidden;text-overflow:ellipsis;white-space:nowrap">
                                    {{ basename($pengajuan->file_persyaratan) }}
                                </span>
                                <span style="color:#b45309;font-size:.65rem">Akan diganti jika upload baru</span>
                            </div>
                            @endif

                            {{-- Tombol tambah file --}}
                            <label for="f-file-syarat-trigger" id="btn-tambah-berkas"
                                style="display:inline-flex;align-items:center;gap:6px;background:#d97706;color:#fff;border-radius:8px;padding:7px 14px;font-size:.75rem;font-weight:700;cursor:pointer">
                    ...
                    <button type="submit" class="btn btn-primary" style="width:100%;justify-content:center;padding:11px;background:#d97706;border-color:#d97706">
                    ...
                    <a href="{{ route('pengajuan.index') }}" class="btn btn-outline" style="width:100%;justify-content:center;margin-top:8px">Batal Edit</a>
                </form>
            </div>
        ...
        <div style="grid-column: 2 / 4">
            ...
            <div class="g-card" style="margin-bottom:12px">
                <div style="padding:10px 18px;display:flex;align-items:center;gap:10px;border-bottom:1px solid #f3f4f6">
                    ...
                    <span style="font-size:.78rem;font-weight:700;color:#374151">Preview Surat (Perbaikan)</span>
                    <span id="live-label" style="font-size:.7rem;color:#9ca3af;margin-left:4px">— Memuat data...</span>
                    <div style="margin-left:auto;display:flex;gap:8px">
                        <span style="font-size:.68rem;color:#9ca3af;background:#f3f4f6;padding:3px 8px;border-radius:99px">Draft Mode</span>
                    </div>
                </div>
                <div style="padding:10px 18px;background:#fafafa">
                    <p style="font-size:.72rem;color:#6b7280">Periksa ulang secara seksama untuk mencegah penolakan dari admin.</p>
                </div>
            </div>
    ...
    <script>
        ...
        // Status Perkawinan — READONLY
        const statusKawinVal = document.getElementById('f-status-kawin-display')?.textContent.trim() || 'Belum Kawin';

        // Set hidden status kawin dari PHP
        ...
        function renderPreview() {
            ...
            document.getElementById('td-status-kawin').textContent = statusKawinVal;
        ...
        const fileSyaratInput = document.getElementById('f-file-syarat');
        if (fileSyaratInput) {
            fileSyaratInput.addEventListener('change', function() {
                if (this.files && this.files[0]) {
                    const maxBytes = 2 * 1024 * 1024;
                    if (this.files[0].size > maxBytes) {
                        alert('Maaf, ukuran file terlalu besar! Maksimal 2 MB.');
                        this.value = '';
                    }
                }
            });
        ...
        (function() {
            ...
            function iconForType(file) {
                ...
                if (ext === 'pdf') return `<span style="background:#fee2e2;color:#b91c1c;border-radius:6px;padding:2px 7px;font-size:.65rem;font-weight:700">PDF</span>`;
            ...
            function renderList() {
                ...
                sizeSpan.textContent = formatSize(selectedFiles.reduce((s, f) => s + f.size, 0));
                ...
                selectedFiles.forEach((file, idx) => {
                    ...
                    row.innerHTML = `${iconForType(file)}
                        ...
                        <button type="button" data-idx="${idx}" style="background:#fee2e2;color:#b91c1c;border:none;border-radius:6px;padding:3px 8px;cursor:pointer;font-size:.7rem;font-weight:700;flex-shrink:0">Hapus</button>`;
            ...
            trigger.addEventListener('change', function() {
                ...
                newFiles.forEach(file => {
                    if (file.size > MAX_BYTES) { rejected.push(`"${file.name}" terlalu besar (maks. 5 MB).`); return; }
                    if (selectedFiles.some(f => f.name === file.name && f.size === file.size)) { rejected.push(`"${file.name}" sudah ada.`); return; }
                    if (selectedFiles.length >= MAX_FILES) { rejected.push(`Maksimal ${MAX_FILES} berkas.`); return; }
                    selectedFiles.push(file);
                });
                if (rejected.length > 0) alert('Perhatian:\n' + rejected.join('\n'));
                this.value = '';
                renderList();
            });
        })();
        ...
        document.getElementById('form-pengajuan').addEventListener('submit', function(e) {
            if (!selEl.value && !document.querySelector('input[name="jenis_surat_id"]')?.value) { e.preventDefault(); alert('Pilih jenis surat terlebih dahulu.'); return; }
            ...
            // Sync hidden jenis — gunakan nilai dari hidden name atau dari selEl
            const hiddenJenisNamed = document.querySelector('input[name="jenis_surat_id"]');
            hiddenJenis.value = selEl.value || (hiddenJenisNamed ? hiddenJenisNamed.value : '');
            hiddenKep.value      = fKeperluan.value;
        ...
        // Init: gunakan nilai dari selEl atau dari hidden input jenis_surat_id
        const initVal = selEl.value || document.querySelector('input[name="jenis_surat_id"]')?.value || '';
        if (initVal) {
            hiddenJenis.value = initVal;
            fetchTemplate(initVal);
            fetchEligibility(initVal);
            updateKepCounter();

            // init panel kematian jika jenis surat = 4
            if (initVal == '4') {
                document.getElementById('label-data-utama').textContent = '(2) Data Almarhum / Orang Yang Meninggal';
                document.getElementById('panel-kematian').style.display = 'block';
            }
